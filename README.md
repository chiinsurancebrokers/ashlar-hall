...
- **🇬🇷 / 🇬🇧 Bilingual Comparison quick-prompt** (HAL chat) — an expander above the chat input with two tabs (Ελληνικά / English). Each tab carries (1) the comparison methodology explainer in its own language and (2) a primary button that, with files staged, sends a fully structured language-matched prompt to HAL through the standard `_send_to_hal()` path. Submission flow is factored into a single nested `_submit_comparison(prompt_body, lang_tag)` helper so the two buttons share one implementation.
- **Bulletproof Greek font handling** in both prompts — earlier output had `■` instead of Greek diacritics (ά έ ή ί ό ύ ώ) because ReportLab's default Helvetica/Times fonts lack the extended-Greek Unicode block. The new shared `_HAL_FONT_BLOCK` (embedded in both EL and EN prompts) walks HAL through four mandatory steps: **discover** a Greek-capable system TTF via `fc-list :lang=el`, **register** it via `pdfmetrics.registerFont(TTFont(...))`, **use** it in every ParagraphStyle/TableStyle/canvas.setFont call (no Helvetica allowed), and **verify** the rendered PDF with PyMuPDF — counting how many of `άέήίόύώΆΈΉΊΌΎΏ` survive and raising `RuntimeError("VERIFICATION FAILED")` to force a font retry if any are still `■`. End-to-end tested locally: `fc-list :lang=el` returns DejaVuSans + Carlito + others on stock Ubuntu, registration + verify works, 14/14 diacritics render. The English-language prompt also carries this block because client/insurer/product names often contain Greek characters even in an English-language PDF.
- Module-level constants renamed `_HAL_COMPARISON_PROMPT_EL` and `_HAL_COMPARISON_PROMPT_EN` (was the single `_HAL_GREEK_COMPARISON_PROMPT`). Sheet log records a short `[Quick-action: Insurance comparison → {lang} PDF]` tag rather than the ~6 KB structured prompt — keeps the rolling-memory window clean across sessions.
...
- **🔄 Retry / Regenerate button** next to "Clear conversation" — covers two failure modes: (1) HAL didn't reply at all (button reads "Retry — get HAL's response") and (2) HAL replied but it was cut off mid-execution, errored, or you simply want a different answer (button reads "Regenerate response"). Pops any trailing assistant turn(s) so the inference re-runs against the same user message — attachments from that turn are preserved, so you don't have to re-upload PDFs to retry a comparison. The retried user message is NOT re-logged to Google Sheets (it was logged when first sent), only the new assistant reply gets logged. The inference logic was factored out of the chat-input handler into a nested `_send_to_hal()` so both paths share one implementation.
