...
def generate_pptx(
    ...
    theme: key into themes.THEMES (e.g. "ocean", "midnight_gold", "forest").
    Falls back to the built-in C palette if themes.py / the key is missing,
    so existing callers that don't pass theme keep their current look.
    """
    if theme:
        try:
            ...
            pass  # themes.py not present or unknown key — keep default C palette
