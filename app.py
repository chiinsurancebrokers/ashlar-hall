...
def _generate_quote_comparison(quote_data: dict, api_key: str, lang: str) -> str:
    """Call Claude to produce a structured insurance plan comparison."""
    import anthropic
    lang_instruction = "Respond in Greek." if lang == "el" else "Respond in English."
...
Client profile collected via voice interview:
{profile}

Based on this profile, provide a concise insurance recommendation. Structure your response as follows:

**RECOMMENDATION** (1-2 sentences spoken recommendation for voice)

**TOP 3 PLANS**
For each plan provide: Insurer | Product | Key benefits | Estimated premium range | Suitability score /10

**KEY CONSIDERATIONS** (2-3 bullet points specific to this client)

**NEXT STEP** (one clear action)

Use your knowledge of: Groupama, Generali, Ethniki (Greek domestic — no outpatient, no dental, no MRI outside hospitalisation, deductible per-hospitalisation or annual); Morgan Price, NOW Health, Bupa Global (international — full outpatient, diagnostics, physio, dental depending on plan).

{lang_instruction} Be direct and specific. No generic advice."""
...
Respond in Greek unless spoken to in English. Conversational — no bullet points."""
    ...
    for _k, _v in [
        ...
        ("quote_mode", False), ("quote_step", 0), ("quote_data", {}),
        ("quote_result", None), ("_last_audio_id", ""), ("_voice_processing", False),
    ]:
    ...
    with qcol1:
        if not st.session_state.quote_mode:
            if st.button("🏥 Start Quote", type="primary", use_container_width=True,
                         help="HAL will ask 6 questions then generate a plan comparison"):
                st.session_state.quote_mode  = True
                st.session_state.quote_step  = 0
                st.session_state.quote_data  = {}
                st.session_state.quote_result = None
                # HAL speaks first question
                first_q = _get_quote_question(0, el_lang)
                intro   = ("Ξεκινάμε τη διαδικασία σύγκρισης. " if el_lang == "el" else "Starting the quote interview. ") + first_q
    ...
    with qcol2:
        if st.session_state.quote_mode and not st.session_state.quote_result:
            ...
            st.progress(min(step / total, 1.0),
                        text=f"Question {min(step+1,total)} of {total} — {QUOTE_QUESTIONS[min(step,total-1)][0].replace('_',' ').title()}")
    ...
    # ── QUOTE MODE: process answer, ask next question or generate comparison ──
    if user_text and st.session_state.quote_mode and not st.session_state.quote_result:
        ...
        if step + 1 < total:
            ...
            # All answers collected — generate comparison
            ...
            with st.spinner("HAL is building your quote comparison…"):
                try:
                    ...
                    # Extract the RECOMMENDATION line to speak
                    ...
                    if not spoken:
                        spoken = " ".join(result.split()[:40])  # first 40 words fallback
