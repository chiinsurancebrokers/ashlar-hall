...
def lookup_premium(carrier, plan, age, area="area1"):
    a = int(age) if str(age).isdigit() else 45
    if carrier == "morgan_price":
        return _MP_RATES.get(area, {}).get(_mp_band(a), {}).get(plan)
    elif carrier == "april":
        return _APR_RATES.get(area, {}).get(_apr_band(a), {}).get(plan)
    elif carrier == "img":
        return _IMG_RATES.get(str(min(a,80)), {}).get(plan)
    return None
